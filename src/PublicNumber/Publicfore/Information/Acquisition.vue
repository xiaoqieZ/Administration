    ...
    <div v-show="values==0">
      ...
      >
        ...
        <el-form-item label="证件类型" prop="certificateTypeName" v-if="inputName">
          <el-select v-model="ruleForm.certificateTypeName" placeholder="请选择证件类型" :disabled="true">
            <el-option label="身份证" value="1"></el-option>
            <el-option label="军人证" value="2"></el-option>
            <el-option label="结婚证" value="3"></el-option>
          </el-select>
      ...
      <div class="radios" >
        <div class="radisFilial " v-for="(item,index) in listOption" :key="item.id">
          <p><span>{{item.id}}、</span>{{item.title}}</p>
    ...
    <div class="radiosplay" v-show="values==1">
      <div class="Congratulations">
        <div class="Congratu">
          <h4>
            恭喜您完成基本信息采集，您将申请成为：
            <span style="color:red">普通投资者</span>
          </h4>
        </div>
      </div>
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) { 
                this.radio=[this.listOption[0].radio,this.listOption[1].radio,this.listOption[2].radio
                ,this.listOption[3].radio,this.listOption[4].radio,this.listOption[5].radio]
          let data = {sex:this.gender,profession:this.occupation,
                      duty:this.post,landline:this.landline,
                      postal:this.code,address:this.address,
                      acquisitionResult:this.radio
                      };
    ...
    getAcquisitionOption(){
      ajax.authGet.bind(this)('/api/Information/Account/AcquisitionOption',res=>{
        console.log(res);
